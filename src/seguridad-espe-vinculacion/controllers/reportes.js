...
const obtenerDatosCards = async (req, res) => {
  try {
    const { unidadEducativa } = req.body;

    // Consultar publicaciones y usuarios que coinciden con la unidad educativa
    const publicaciones = await Publicacion.find({ unidadEducativa });
    const usuarios = await Usuario.find({ unidadEducativa });

    // Obtener el total de publicaciones y usuarios registrados con el filtro de unidad educativa
    const publicacionesRegistradas = publicaciones.length;
    const usuariosRegistros = usuarios.length;

    // Obtener el total de publicaciones del mes para la unidad educativa
    ...
    const publicacionesDelMes = await Publicacion.countDocuments({
      createdAt: { $gte: primerDiaMesActual, $lte: ultimoDiaMesActual },
      unidadEducativa
    });

    // Obtener el total de publicaciones del día para la unidad educativa
    const fechaInicioDiaActual = new Date(fechaActual.getFullYear(), fechaActual.getMonth(), fechaActual.getDate());
    const fechaFinDiaActual = new Date(fechaActual.getFullYear(), fechaActual.getMonth(), fechaActual.getDate(), 23, 59, 59, 999);
    const publicacionesDelDia = await Publicacion.countDocuments({
      createdAt: { $gte: fechaInicioDiaActual, $lte: fechaFinDiaActual },
      unidadEducativa
    });

    res.json({
      ok: true,
      msg: "Datos de las cards obtenidos correctamente",
      data: {
...
// TODO : obtenerDatosCardsF3  
const obtenerDatosCardsF3 = async (req, res) => {
  try {
    const { unidadEducativa, primerDiaMesActual, ultimoDiaMesActual, fechaInicioDiaActual, fechaFinDiaActual } = req.body;
    ...
    // Consultar las publicaciones que se encuentran dentro del rango del mes actual y coinciden con la unidad educativa
    const publicacionesDelMes = await Publicacion.countDocuments({
      createdAt: { $gte: primerDiaMesActual, $lte: ultimoDiaMesActual },
      unidadEducativa
    });

    // Consultar las publicaciones que se encuentran dentro del rango del día actual y coinciden con la unidad educativa
    const publicacionesDelDia = await Publicacion.countDocuments({
      createdAt: { $gte: fechaInicioDiaActual, $lte: fechaFinDiaActual },
      unidadEducativa
    });
