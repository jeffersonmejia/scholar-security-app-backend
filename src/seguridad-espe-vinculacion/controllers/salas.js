...
const unirseSala = async (req, res) => {
  ...
  try {
    ...
    const usuarioPropietario = await Usuario.findById(uid);

    const usuarioUnido = await Usuario.findById(sala.propietario.toString());

    //Si no pertecen a la misma unidadEducativa no se puede unir
    if (usuarioPropietario.unidadEducativa.toString() !== usuarioUnido.unidadEducativa.toString()) {
      return res.status(404).json({
        ok: false,
        msg: "No pertenecen a la misma unidad educativa",
      });
    }
